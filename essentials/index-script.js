...
window.__gcse = {
  ...
  const observer = new MutationObserver(() => {
    ...
    const existingFallback = document.getElementById('no-results-message');

    if (results.length === 0) {
      if (!existingFallback) {
        const fallback = document.createElement('div');
        fallback.id = 'no-results-message';
        fallback.innerHTML = `
        ...
        `;
        document.querySelector('#gcse-results')?.appendChild(fallback);
      }
      return;
    ...
    results.forEach(result => {
      ...
      const url = result.querySelector('.gs-visibleUrl');

      if (!titleLink || !url) return;
      ...
      block.className = 'google-like-result';

      block.innerHTML = `
        <div class="google-site-line">
          <img src="https://${domain}/favicon.ico" class="google-favicon" onerror="this.remove()">
        ...
        <div class="google-snippet">
          ${snippet ? snippet.innerHTML : ''}
        </div>
      `;

      result.parentNode.insertBefore(block, result.nextSibling);
